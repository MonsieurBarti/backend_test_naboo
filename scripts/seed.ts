...
const MONGODB_URI = 'mongodb://localhost:27017/event-scheduling?replicaSet=rs0';
...
async function run(): Promise<void> {
	...
	const BusyCorpEventModel = conn.model<EventDocument>(`${org1Slug}_Event`, EventSchema, `${org1Slug}_events`);
	const BusyCorpOccurrenceModel = conn.model<OccurrenceDocument>(
		`${org1Slug}_Occurrence`,
		OccurrenceSchema,
		`${org1Slug}_occurrences`,
	);
	const MinimalOrgEventModel = conn.model<EventDocument>(`${org2Slug}_Event`, EventSchema, `${org2Slug}_events`);
	...
	console.log('\n  Organizations (use as x-tenant-id):');
	console.log(`    Busy Corp   : ${org1Id}`);
	console.log(`    Minimal Org : ${org2Id}`);
	console.log('\n  Edge cases seeded:');
